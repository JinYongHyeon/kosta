...
public class BoardDAO {
	...
	public int getBoardAllRow() throws SQLException {
		int total = 0;
		Connection con =null;
	 ...
	 * 
	 */
	public ArrayList<PostVO> getPostingList(PagingBean bean) throws SQLException{
		...
		try{
			...
			sql.append("FROM(SELECT ROW_NUMBER() OVER(ORDER BY no DESC) AS rnum, no,title,hits,to_char(time_posted,'YYYY.MM.DD') as time_posted,id FROM board)");
			sql.append("b,board_member m WHERE b.id=m.id AND b.rnum BETWEEN ? AND ?");	
			pstmt=con.prepareStatement(sql.toString());			
			pstmt.setInt(1, bean.getPageStartRow());
			pstmt.setInt(2, bean.getPageEndRow());
			rs=pstmt.executeQuery();	

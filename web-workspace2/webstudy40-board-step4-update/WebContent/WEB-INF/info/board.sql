...
FROM(SELECT ROW_NUMBER() OVER(ORDER BY no DESC) AS rnum, no,title,hits,to_char(time_posted,'YYYY.MM.DD') as time_posted,id FROM board)
b,board_member m WHERE b.id=m.id AND b.rnum BETWEEN 5 AND 10

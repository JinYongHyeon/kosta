 ...
 * DBCP -> 시스템 성능 향상을 위해 DB 컨넥션을 생성, 소멸시키지 않고 미리 생성한 컨넥션을 빌려주고
 * 반납하도록 한다.
 * 
 * import javax.sql.DataSource : 타입으로 DBCP를 관리한다.
 * 이유는 WAS가 변경되면 dbcp도 변경될 수 있으므로 추상화된 인터페이스 타입으로 관리하는 것이
 * 유지보수에 유리하다.
 */
public class DataSourceManager {
	...
	private DataSourceManager() {
		...
		dbcp.setInitialSize(3); //처음 로드될 때 생성해 놓을 컨넥션 수 : 예상인원을 생각해서 미리 만들어놈

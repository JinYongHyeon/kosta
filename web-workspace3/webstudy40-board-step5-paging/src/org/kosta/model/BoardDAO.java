...
public class BoardDAO {
	 ...
	 * step2:
	 * 
	 * SELECT b.*,m.name FROM( SELECT ROW_NUMBER() OVER(ORDER BY NO DESC) AS
	 * rnum,no,title,hits, TO_CHAR(time_posted,'YYYY.MM.DD') AS time_posted,id FROM
	 * board )b,board_member m WHERE m.id =b.id AND rnum BETWEEN 1 AND 5
	 * 
	 * 
	 * @param pageNo
	 * @return
	 * @throws SQLException
	 */
	public ArrayList<PostVO> getPostingList(PagingBeanEX pagingBean) throws SQLException {
		...
		try {
			...
			sql.append("SELECT b.no,b.title,b.hits,b.time_posted,m.id,m.name FROM(");
			sql.append("SELECT ROW_NUMBER() OVER(ORDER BY NO DESC) AS rnum,no,title,hits,");
			sql.append("TO_CHAR(time_posted,'YYYY.MM.DD') AS time_posted,id FROM board)b,board_member m");
